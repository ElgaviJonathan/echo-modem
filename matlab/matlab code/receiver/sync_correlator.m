function [signal_detected, end_index, corr_max] = sync_correlator(input_signal, gold_code_signal, threshold, start_index, coherent_detection, passband_freq, sample_rate)
...
for i = max((start_index-seq_len+1),1):step_size:(sig_len - seq_len) % from possible start of seq to possible end
    ...
    if(corr_test >= internal_threshold && corr_test >= corr_max)
        corr_max = corr_test;
...
% Additional search to find peak betweem spars peak searches
for j = max(start_index+1-step_size*5,0):min(start_index+1+step_size*5, (sig_len - seq_len))
